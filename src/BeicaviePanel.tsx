...
export function BeicaviePanel(props: Props) {
  ...
  useEffect(() => {
    axios.get(
      'http://127.0.0.1:8888/devices/133e7712-ace0-4631-a06c-daa90c038fb6',
      {
        headers: {
  ...
  const saveAnnotation = (event: any) => {
    if (annotation) {
      ...
      // modify annotation
      axios.post(
        'http://127.0.0.1:8888/devices/133e7712-ace0-4631-a06c-daa90c038fb6/annotations',
        {
          Description: description,
          Data: {
            Hives: hives
  ...
  return (
    ...
    >
      <h3 style={{textAlign: "center"}}>{options.title} {device?.Device}</h3>
      {device && !editing && <>
        <div
          style={{
            textAlign: "right",
        ...
        >
          <Button variant="transparent" onClick={() => setEditing(true)}>Modifica</Button>
        </div>
        <h3
          style={{
            textAlign: "center",
      ...
      {device && editing && <>
        ...
        >
          ...
          <FormField label="Note" accept="number" value={description} onChange={handleDescriptionChange} />
        ...
        >
          <Button variant="transparent" onClick={() => setEditing(false)}>Annulla</Button>
